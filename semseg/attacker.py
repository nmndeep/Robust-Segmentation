...
def apgd_train(
    ...
    m_acc, a_acc, m_iou = compute_iou_acc(pred_best, y, n_cls, ignore_index=ignore_index)
    ...
    for i in range(n_iter):
        ...
        if True:  # with torch.no_grad()
            ...
            if norm == "Linf":
                ...
                x_adv_1 = x_adv + step_size * grad / (L2_norm(grad, keepdim=True) + 1e-12)
                x_adv_1 = torch.clamp(
                    x
                    + (x_adv_1 - x)
                ...
                topk_curr = torch.clamp((1.0 - topk) * n_fts, min=0, max=n_fts - 1).long()
