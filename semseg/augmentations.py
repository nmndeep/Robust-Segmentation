...
class CenterCrop:
    def __init__(self, size: int | list[int] | tuple[int]) -> None:
        """
...
class RandomCrop:
    def __init__(self, size: int | list[int] | tuple[int], p: float = 0.5) -> None:
        """
...
class Pad:
    def __init__(self, size: list[int] | tuple[int] | int, seg_fill: int = 0) -> None:
        """
    ...
    def __call__(self, img: Tensor, mask: Tensor) -> tuple[Tensor, Tensor]:
        padding = (
            0,
            0,
            self.size[1] - img.shape[2],
            self.size[0] - img.shape[1],
        )
        return TF.pad(img, padding), TF.pad(mask, padding, self.seg_fill)


class ResizePad:
    def __init__(self, size: int | tuple[int] | list[int], seg_fill: int = 0) -> None:
        """
...
class Resize:
    def __init__(self, size: int | tuple[int] | list[int]) -> None:
        """
        Resize the input image to the given size.
    ...
    def __call__(self, img: Tensor, mask: Tensor) -> tuple[Tensor, Tensor]:
        ...
        alignH, alignW = (
            int(math.ceil(nH / 32)) * 32,
            int(math.ceil(nW / 32)) * 32,
        )
...
def get_train_augmentation(size: int | tuple[int] | list[int], seg_fill: int = 0):
    return Compose([
...
def get_val_augmentation(size: int | tuple[int] | list[int]):
    return Compose([Resize(size), CenterCrop(size)])
