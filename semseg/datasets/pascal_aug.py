...
class VOCAugSegmentation(SegmentationDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.masks = []
        self.fnames = []
        with open(os.path.join(_split_f)) as lines:
            for line in lines:
                _image = os.path.join(_image_dir, line.rstrip("\n") + ".jpg")
                assert os.path.isfile(_image)
                self.images.append(_image)
                self.fnames.append(line.rstrip("\n") + ".jpg")
    ...
    def __getitem__(self, index):
        ...
        if self.mode == "train":
            ...
            raise RuntimeError(f"unknown mode for dataloader: {self.mode}")
        # general resize, normalize and toTensor
        if self.transform is not None:
            img = self.transform(img)
        return img, target, self.fnames[index]
