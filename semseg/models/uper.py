...
import torch.nn as nn
import numpy as np
from semseg.models.backbones.convnext_orig import ConvNeXt, LayerNorm
...
class ResNet(nn.Module):
    def __init__(self, in_channels=3, output_stride=16, backbone='resnet101', pretrained=True):
        ...
        model = getattr(models.backbones, backbone)(pretrained)
...
class FPN_fuse(nn.Module):
    def __init__(self, feature_channels=[256, 512, 1024, 2048], fpn_out=256):
        ...
        self.conv_fusion = nn.Sequential(
            nn.Conv2d(len(feature_channels)*fpn_out, 256, kernel_size=3, padding=1, bias=False),
            nn.BatchNorm2d(256),
            nn.ReLU(inplace=True)
        )
...
def load_checkpoint(model, pretrained):    
    ...
    # print(ckpt['stem'].keys()) 
    try:
        model.backbone.downsample_layers.stem.load_state_dict(hasattr(ckpt, stem), strict=False)
        model.backbone.stages.load_state_dict(hasattr(ckpt, stem), strict=False)
...
class UperNet(BaseModel):
    ...
    def __init__(self, backbone, num_classes, pretrained, use_aux=True, fpn_out=96, freeze_bn=False, **_):
        ...
        self.backbone = ConvNeXt(variant, drop_path_rate=0.3, layer_scale_init_value=1.0, out_indices=[0, 1, 2, 3])
    ...
    def get_decoder_params(self):
        return [self.PPN, self.FPN, self.head]

    def freeze_bn(self):
        for module in self.modules():
            if isinstance(module, nn.BatchNorm2d): module.eval()
            if isinstance(module, LayerNorm): module.eval()
