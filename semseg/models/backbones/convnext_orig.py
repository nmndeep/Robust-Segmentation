...
class ConvNeXt(nn.Module):
    ...
    def __init__(
        ...
        assert (
            strr in CONVNEXT_SETTINGS.keys()
    ...
    def load_carefully_cvst(self, pretrained):
        ...
        with torch.no_grad():
            ...
            for l in range(1, 4):
                for p in range(2):
                    self.downsample_layers[l][p].weight.copy_(
                        ckpt[f"stages.{l}.downsample.{p}.weight"]
                    )
                    self.downsample_layers[l][p].bias.copy_(
                        ckpt[f"stages.{l}.downsample.{p}.bias"]
                    )
