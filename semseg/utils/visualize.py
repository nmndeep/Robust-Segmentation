...
# def visualize_dataset_sample(root='/scratch/nsingh/datasets', split='val', batch_size=4, seg_maps):
...
#     val_dataset = get_segmentation_dataset('ADE20K', root=root, split='val', mode='val', **data_kwargs)
#     dataloader = DataLoader(val_dataset, shuffle=True, batch_size=batch_size)
...
#     plt.imshow(make_grid(images, nrow=4).to(torch.uint8).numpy().transpose((1, 2, 0)))
    # plt.savefig("/scratch/nsingh/model_zoo/seg_models/tester_image.png")
...
col_dict={'150':colors_ade, '21':colors_voc}

def generate_palette(num_classes, background: bool = False):
    colors = col_dict[f'{num_classes}']
